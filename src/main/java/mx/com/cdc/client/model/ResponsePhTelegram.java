package mx.com.cdc.client.model;

import java.util.Objects;
import java.util.Arrays;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;
@ApiModel(description = "Detalle del número telefónico con alguna cuenta existente de telegram.")


public class ResponsePhTelegram {
  @SerializedName("registered")
  private Boolean registered = null;
  @SerializedName("last_seen")
  private String lastSeen = null;
  @SerializedName("photo")
  private String photo = null;
  public ResponsePhTelegram registered(Boolean registered) {
    this.registered = registered;
    return this;
  }
   
  @ApiModelProperty(value = "Indica si el número telefónico se encuentra registrado.")
  public Boolean isRegistered() {
    return registered;
  }
  public void setRegistered(Boolean registered) {
    this.registered = registered;
  }
  public ResponsePhTelegram lastSeen(String lastSeen) {
    this.lastSeen = lastSeen;
    return this;
  }
   
  @ApiModelProperty(example = "1659560211", value = "Timestamp de la última actividad en línea, en formato UNIX y zona horaria UTC.")
  public String getLastSeen() {
    return lastSeen;
  }
  public void setLastSeen(String lastSeen) {
    this.lastSeen = lastSeen;
  }
  public ResponsePhTelegram photo(String photo) {
    this.photo = photo;
    return this;
  }
   
  @ApiModelProperty(example = "", value = "Imagen codificada en Base64 del usuario.")
  public String getPhoto() {
    return photo;
  }
  public void setPhoto(String photo) {
    this.photo = photo;
  }
  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    ResponsePhTelegram responsePhTelegram = (ResponsePhTelegram) o;
    return Objects.equals(this.registered, responsePhTelegram.registered) &&
        Objects.equals(this.lastSeen, responsePhTelegram.lastSeen) &&
        Objects.equals(this.photo, responsePhTelegram.photo);
  }
  @Override
  public int hashCode() {
    return Objects.hash(registered, lastSeen, photo);
  }
  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class ResponsePhTelegram {\n");
    
    sb.append("    registered: ").append(toIndentedString(registered)).append("\n");
    sb.append("    lastSeen: ").append(toIndentedString(lastSeen)).append("\n");
    sb.append("    photo: ").append(toIndentedString(photo)).append("\n");
    sb.append("}");
    return sb.toString();
  }
  
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }
}
